...
public class StepArcView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        /**指定圆弧的外轮廓矩形区域*/
        RectF rectF = new RectF(0+borderWidth,0+borderWidth,2*centerX-borderWidth,2*centerX-borderWidth);
        /**【第一步】绘制整体的黄色圆弧*/
    ...
    private void drawTextNumber(Canvas canvas, float centerX) {
        ...
        vTextPaint.setTypeface(typeface);
        vTextPaint.setColor(getResources().getColor(R.color.colorAccent));
        Rect bound_nums = new Rect();
        vTextPaint.getTextBounds(stepNumber,0,stepNumber.length(),bound_nums);
        canvas.drawText(stepNumber,centerX,getHeight() / 2 - bound_nums.height() /2,vTextPaint);
    }
    ...
    private void drawTextStepString(Canvas canvas, float centerX) {
        Paint vTextPaint = new Paint();
        vTextPaint.setTextSize(Utils.dipToPx(getContext(),16));
        vTextPaint.setTextAlign(Paint.Align.CENTER);
    ...
    public void setCurrentAngleLength(int totalStepNum, int currentCounts){
        ...
        float scan = (float) currentCounts/totalStepNum;
        /**换算成弧度最后要到达的角度的长度-->弧长*/
        float currentAngleLength = scan * angleLength;
        /**开始执行动画*/
    ...
    public void setTextSize(int num) {
        ...
        int length = s.length();
        if (length <= 4) {
            numberTextSize = Utils.dipToPx(getContext(),50);
        } else if (length > 4 && length <= 6) {
            numberTextSize = Utils.dipToPx(getContext(),40);
        } else if (length > 6 && length <= 8) {
            numberTextSize = Utils.dipToPx(getContext(),30);
        } else if (length > 8) {
            numberTextSize = Utils.dipToPx(getContext(),25);
        }
    }
}
