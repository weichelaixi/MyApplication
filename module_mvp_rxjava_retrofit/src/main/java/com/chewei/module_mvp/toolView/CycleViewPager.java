package com.chewei.module_mvp.toolView;
...
import com.chewei.module_mvp.R;
import com.chewei.module_mvp.toolView.rotate_viewpager.ActivityImageBean;
import com.chewei.module_mvp.toolView.rotate_viewpager.BaseViewPager;
import com.chewei.module_mvp.toolView.rotate_viewpager.CycleViewPagerHandler;
...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
	...
	String[] indicatorsTypeStr;
	private FrameLayout viewPagerFragmentLayout;
	private LinearLayout indicatorLayout; // 指示器
	private TextView indicatorTv; //指示器文字
	private TextView type_tv;//类型文本
	...
	private List<ActivityImageBean> infos;
	int pageMargin = 0;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		indicatorTv = (TextView) view.findViewById(R.id.layout_viewpager_text);
		type_tv =  (TextView) view.findViewById(R.id.type_tv);
//		indicatorTv.getBackground().setAlpha(150);//0~255透明度值
	...
	public void setData(List<ImageView> views, List<ActivityImageBean> list, ImageCycleViewListener listener) {
	 ...
	 *            默认显示位置
	 */
	public void setData(List<ImageView> views, List<ActivityImageBean> list, ImageCycleViewListener listener, int showPosition) {
		...
		indicatorsStr = new String[ivSize];
		indicatorsTypeStr= new String[ivSize];
		if (isCycle) {
			indicatorsStr = new String[ivSize - 2];
			indicatorsTypeStr = new String[ivSize - 2];
		...
		for (int i = 0; i < infos.size(); i++) {
			if (!TextUtils.isEmpty(infos.get(i).getName())) {
				indicatorsStr[i] = infos.get(i).getName();
			}
			if (!TextUtils.isEmpty(infos.get(i).getTypeName())) {
				indicatorsTypeStr[i] = infos.get(i).getTypeName();
			}
		}
	 ...
	 * 是否显示指示器
	 */
	public void setVisibleIndicatorLayout(boolean isVisible){
		indicatorLayout.setVisibility(isVisible ? View.VISIBLE: View.GONE);
	}
	...
	private void setIndicator(int selectedPosition) {
		for (int i = 0; i < indicators.length; i++) {
			indicators[i].setBackgroundResource(R.drawable.circle_common_point);
		}
		if (indicators.length > selectedPosition) {
			if (!TextUtils.isEmpty(indicatorsStr[selectedPosition])) {
				indicatorTv.setText(indicatorsStr[selectedPosition]);
			}
		}
		if (indicators.length > selectedPosition) {
			if (!TextUtils.isEmpty(indicatorsTypeStr[selectedPosition])) {
				type_tv.setVisibility(View.VISIBLE);
				type_tv.setText(indicatorsTypeStr[selectedPosition]);
			} else {
				type_tv.setVisibility(View.GONE);
			}
		}

		indicators[selectedPosition].setBackgroundResource(R.drawable.circle_red_point);
	}
	...
	public static interface ImageCycleViewListener {
		...
		public void onImageClick(ActivityImageBean info, int postion, View imageView);
	}
